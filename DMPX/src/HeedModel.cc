...
// We need the pressure in Torr for Magboltz 
const static G4double torr = 1. / 760. * atmosphere;

HeedModel::HeedModel(GasModelParameters* gmp, G4String modelName, G4Region* envelope, 
                     DetectorConstruction* dc, GasBoxSD* sd)
                     : G4VFastSimulationModel(modelName, envelope),                             
                     detCon(dc), fGasBoxSD(sd), fGasModelParameters(gmp){
...
void HeedModel::SettingChamberView(){
    G4cout << "(Debug: HeedModel.cc) Setting chamber view..." << G4endl;
...
void HeedModel::SettingSignalView(){
    G4cout << "(Debug: HeedModel.cc) Setting signal view..." << G4endl;
...
void HeedModel::SettingFieldView(){
    G4cout << "(Debug: HeedModel.cc) Setting field view..." << G4endl;
...
void HeedModel::Drift(double x, double y, double z, double t) {
    ...
    if (driftElectrons) {
        ...
        // electron generated by Degrad)
            ...
            G4cout << "(Debug: HeedModel.cc) Using microscropic tracking for the drift..." << G4endl;
            ...
            for (int i=0; i < nLines; i++) {
                ...
                // This snippet determines which anode did the electron touch
                if (status == -5) { // If the status is -5, we are inside a wire (see Garfield++ documentation)
                    double min_distance = anodesSpacing * 1e5; // Setting a big distance compared to our geometry
                    int index = -1;
                    for (int i = 0; i < nbOfAnodes; i++) {
                    const double xPos = anodesSpacing * (i - nbOfAnodes / 2);
                    if (abs(xPos - xfinal) < min_distance) {
                        min_distance = abs(xPos - xfinal);
                        index = i;
                    }
                    }
                    G4cout << "(Debug: HeedModel.cc) The wire touched by the electron is: " << index << G4endl;
                    electronsInWires[index]++; // Increment the number of electrons in the wire reached by the electron
                    secondaryElectronCounter++; // The secondary electron generated by Degrad has touched the wire!  
                ...
                if (G4VVisManager::GetConcreteInstance() && j % jumpDriftStepPoints == 0) { // To get all the drift step points or only some of them
                    dlt->AppendStep(G4ThreeVector(xi * CLHEP::cm, yi * CLHEP::cm, zi * CLHEP::cm), ti);
                    G4cout << "(Debug: HeedModel.cc) Appended step: " << G4BestUnit(xi, "Length") << " " 
                    << G4BestUnit(yi, "Length") << " " << G4BestUnit(zi, "Length") << G4endl;
                }
                }
            ...
            G4cout << "(Debug: HeedModel.cc) Using AvalancheMC for the drift..." << G4endl;
            ...
            for (unsigned int i = 0; i < nLines; i++) {
                ...
                dlt->AppendStep(G4ThreeVector(xi * CLHEP::cm, yi * CLHEP::cm, zi * CLHEP::cm), ti);
                G4cout << "(Debug: HeedModel.cc) Endpoint: " << G4BestUnit(xi, "Length") << " "
                << G4BestUnit(yi, "Length") << " " << G4BestUnit(zi, "Length") <<  "| Status: " << status << G4endl;
