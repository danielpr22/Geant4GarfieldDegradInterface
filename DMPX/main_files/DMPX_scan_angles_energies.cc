...
int main(int argc, char** argv) {
  ...
  if (argc == 1)  //! define UI terminal for interactive mode:
  {
    ...
    UImanager->ApplyCommand("/control/execute run_files/scan_angles_energies.mac");
    ...
    std::vector<double> energies = {14.0}; // in keV
    std::vector<double> angles = {70.0}; // in degrees
